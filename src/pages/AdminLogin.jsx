...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Login response:", response.data); // Debug: Log the full response

      // Check for accessToken (server returns accessToken, not token)
      if (response.data && response.data.accessToken) {
        localStorage.setItem("token", response.data.accessToken); // Store the token
        console.log("Token stored:", response.data.accessToken); // Debug: Confirm token

        // Store user data if provided (optional)
        if (response.data.user) {
          localStorage.setItem("user", JSON.stringify(response.data.user));
          console.log("User stored:", response.data.user); // Debug: Confirm user data
        }
